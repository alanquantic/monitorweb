...
export default async function handler(req, res) {
  console.log('🚀 Iniciando monitoreo Grupo 3 (sitios 11-13)...');
  ...
  try {
    ...
    for (const site of sitesGroup3.filter(s => s.enabled)) {
      let browser = null;
      try {
    ...
    const report = {
      timestamp: new Date().toLocaleString('es-ES'),
      group: "Grupo 3 (Sitios 11-13)",
      totalSites: sitesGroup3.length,
    ...
    if (process.env.MAILGUN_DOMAIN && process.env.MAILGUN_TO) {
      ...
      const subject = failedSites.length > 0 
        ? `🚨 Monitor Web ${currentTime} GRUPO 3 - ${failedSites.length} sitios con problemas`
        : `✅ Monitor Web ${currentTime} GRUPO 3 - Todos funcionando (${report.summary.uptime}% uptime)`;
      ...
      messageData.attachment.push({
        filename: `reporte_grupo3_${new Date().toISOString().split('T')[0]}.json`,
      ...
      console.log(`📧 Email Grupo 3 enviado: ${emailResponse.id}`);
    }

    // Respuesta exitosa
    res.status(200).json({
      success: true,
      message: 'Monitoreo Grupo 3 completado exitosamente',
      report: report
    });

  } catch (error) {
    console.error('❌ Error en monitoreo Grupo 3:', error);
    res.status(500).json({
      success: false,
      error: error.message
    });
  ...
  return `
    <div style="font-family: Arial, sans-serif; max-width: 800px; margin: 0 auto;">
      <h2 style="color: #333; border-bottom: 2px solid #dc3545; padding-bottom: 10px;">
        📊 Reporte Monitor Web - GRUPO 3 (Sitios 11-13)
      </h2>
      
      <div style="background-color: #f8f9fa; padding: 15px; border-radius: 5px; margin: 20px 0;">
        <h3 style="color: #495057; margin-top: 0;">📈 Resumen Grupo 3</h3>
        <p><strong>📅 Fecha:</strong> ${timeZone}</p>
        <p><strong>🌐 Sitios grupo 3:</strong> ${report.totalSites}</p>
        ...
        <p><strong>📊 Uptime grupo 3:</strong> <span style="color: ${report.summary.uptime >= 95 ? '#28a745' : '#dc3545'}; font-weight: bold; font-size: 18px;">${report.summary.uptime}%</span></p>
      ...
      <div style="background-color: #d4edda; border: 1px solid #c3e6cb; padding: 15px; border-radius: 5px; margin: 20px 0;">
        <h3 style="color: #155724; margin-top: 0;">✅ Sitios Funcionando - Grupo 3</h3>
      ...
      <div style="background-color: #f0f8ff; border: 1px solid #b3d9ff; padding: 15px; border-radius: 5px; margin: 20px 0;">
        <h3 style="color: #0066cc; margin-top: 0;">📎 Adjuntos Grupo 3</h3>
        <p>📸 Capturas de pantalla de sitios funcionando</p>
        <p>📊 Reporte JSON con métricas del Grupo 3</p>
      </div>

      <div style="text-align: center; margin-top: 30px; padding-top: 20px; border-top: 1px solid #dee2e6;">
        <small style="color: #6c757d;">
          Monitor Web Grupo 3 - Vercel + Mailgun - ${new Date().getFullYear()}
        </small>
      </div>
    </div>
  `;
